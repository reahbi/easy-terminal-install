...
| **Claude Code** | AI에게 작업 지시 (`cc` = `claude --dangerously-skip-permissions` 단축 명령) |
...
`cc`는 `claude --dangerously-skip-permissions`의 단축 명령입니다.
AI가 확인 없이 자동으로 작업을 수행하는 모드입니다.

```
pro        ← 프로젝트 폴더로 이동
cc         ← Claude Code 실행 (대화형으로 시작)
```

실행되면 AI에게 말하듯이 지시하면 됩니다:
```
> 이 프로젝트 구조 설명해줘
> README.md 한국어로 번역해줘
> 테스트 코드 작성해줘
...
Claude Code를 여러 개 띄워서 동시에 작업할 수 있습니다:
...
│ > 코드 리뷰해줘      │ > 테스트 작성해줘     │
│                      │                      │
│ AI가 코드 분석 중... │ AI가 테스트 생성 중...│
│                      │                      │
└──────────────────────┴──────────────────────┘
  Alt+n으로 오른쪽 분할 → 새 pane에서 cc 실행
```

1. `cc` 입력 → Claude Code 실행
2. `Alt+n` → 오른쪽에 새 pane 생성
3. 새 pane에서 `cc` 입력 → 두 번째 Claude Code 실행
4. `Alt+방향키`로 pane 간 이동
...
│                      │             │ 여기 표시  │
└──────────────────────┴─────────────────────────┘
  Alt+e로 파일 탐색기 열기 → .md 파일에 커서 올리면 미리보기
```

1. `cc` 입력 → Claude Code 실행
2. `Alt+e` → 오른쪽에 파일 탐색기 + 문서 뷰어 열기
3. `.md` 파일에 커서를 올리면 오른쪽에 렌더링된 미리보기 표시
4. `Alt+e` 다시 누르면 닫기
...
| 명령어 | 동작 |
|--------|------|
| `cc` | Claude Code 실행 (`claude --dangerously-skip-permissions`) |
| `yy` | 파일 탐색기 열기 (종료 시 해당 폴더로 이동) |
| `mdv 파일.md` | Markdown 문서를 터미널에서 보기 |
...
> 왼쪽: Claude Code 세션 | 가운데: 파일 탐색기 | 오른쪽: 문서 미리보기 (Markdown 자동 렌더링)
